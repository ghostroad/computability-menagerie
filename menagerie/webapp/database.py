...
menagerie.addImplication(LowKurtz, BND1, DirectJustification(u'The degrees that are low for Kurtz randomness are exactly the hyperimmune-free and non-DNC ones [Greenberg, Miller 2009]. However, every positive density point is Kurtz random, so it is either ML-random (in which case, it is DNC), or hyperimmune (every hyperimmune-free Kurtz random is weak 2-random [Yu]).'))
...
menagerie.addNonimplication(NotDNC, BND1, CompositeJustification(Low1Rand.implies(NotDNC), Low1Rand.doesNotImply(BND1)))
...
menagerie.addNonimplication(BN1R, BND1, CompositeJustification(Low1Rand.implies(BN1R), Low1Rand.doesNotImply(BND1)))
